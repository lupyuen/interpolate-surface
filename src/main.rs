...
static X_VIRTUAL_GRID: &[[f64; X_PHYSICAL_SUBDIVISIONS + 1]; Y_PHYSICAL_SUBDIVISIONS + 1] = & 
[[0.0; X_PHYSICAL_SUBDIVISIONS + 1]; Y_PHYSICAL_SUBDIVISIONS + 1]
;

static Y_VIRTUAL_GRID: &[[f64; X_PHYSICAL_SUBDIVISIONS + 1]; Y_PHYSICAL_SUBDIVISIONS + 1] = &
[[0.0; X_PHYSICAL_SUBDIVISIONS + 1]; Y_PHYSICAL_SUBDIVISIONS + 1]
;
        ...
        for x in 0..=X_VIRTUAL_SUBDIVISIONS {
            ...
            if let Some((left, top, right, bottom)) = bounding_box {
                if left as u8 != right as u8 || top as u8 != bottom as u8 {
                    print!("****");  //  Flag out multiple Physical Points to a single Virtual Point
                }
            }
